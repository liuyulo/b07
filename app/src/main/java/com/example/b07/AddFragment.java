...
public class AddFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        ((AppCompatActivity) getActivity()).getSupportActionBar().setTitle("Add Courses to " + title);
        ...
        CourseAdapter adapter = new CourseAdapter(
            () -> switch (Student.current) {
                case TAKEN -> a.stream().filter(c -> !t.contains(c)).collect(Collectors.toCollection(TreeSet::new));
                case WISHLIST -> a.stream().filter(c -> !w.contains(c) && !t.contains(c)).collect(Collectors.toCollection(TreeSet::new));
            },
            "+",
